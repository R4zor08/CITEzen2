...
export function StudentDashboard({
  ...
  const recentConcerns = [...myConcerns].
  sort(
    (a, b) =>
  ...
  const stats = {
    ...
    pending: myConcerns.filter((c: any) => c.status === 'pending').length,
    inProgress: myConcerns.filter((c: any) => c.status === 'in-progress').
    length,
    resolved: myConcerns.filter((c: any) => c.status === 'resolved').length
  };
        ...
        className="space-y-5 sm:space-y-8">
          ...
          <motion.div variants={itemVariants}>
            ...
            <p className="text-sm sm:text-base text-gray-400">
              Here's an overview of your submitted concerns.
            </p>
          </motion.div>
          ...
          <div className="glass-panel p-6 sm:p-8 text-center flex flex-col items-center justify-center">
                <div className="h-12 w-12 sm:h-16 sm:w-16 rounded-full bg-white/5 flex items-center justify-center mb-3 sm:mb-4">
                  <FileTextIcon className="h-6 w-6 sm:h-8 sm:w-8 text-gray-500" />
                </div>
                <h3 className="text-base sm:text-lg font-medium text-white mb-1.5 sm:mb-2">
                  No concerns yet
                </h3>
                <p className="text-sm text-gray-400 mb-4 sm:mb-6 max-w-md">
                  You haven't submitted any concerns. If you have any issues,
                  requests, or suggestions, feel free to submit one.
                </p>
                <button
              onClick={() => setActiveTab('submit')}
              className="px-5 sm:px-6 py-2 sm:py-2.5 rounded-xl bg-purple-600 text-white text-sm font-medium hover:bg-purple-500 transition-colors">
        ...
        onSubmit={async (data) => {
          try {
            ...
            toast.success('Concern submitted successfully!', {
              description:
                'Your concern has been routed to the appropriate department.'
            });
